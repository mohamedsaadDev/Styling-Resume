      ...
      <Accordion.Body className="cursor-pointer">
        {
          skills.skills.map((skill,index)=>
            <div key={index} className="my-2 p-3 py-4 border border-secondary rounded">
          <div className="my-2">
            <label className="label-form-choose" for="skills">
              {t("skills")}
            </label>
            <input 
              onChange={(e) => handleInputChange(index, 'titleSkill', e.target.value)}
              className="input-form-choose w-95" type="text" id="skills" />
          </div>
          <div>
            <div className="d-flex align-items-center">
              <Form.Label>{t("level")}</Form.Label>
              <Form.Range
                className="form-range w-75 bg-transparent mx-3"
                min="0"
                max="5"
                step="1"
                value={skill.rangeSkill}
                onChange={(e) => handleInputChange(index, 'rangeSkill', e.target.value)}
              />
            </div>
            <div className="d-flex  flex-row-reverse mt-3">
              <button
                className="btn-add hvr-sweep-to-left"
              > {t('save')}</button>
              <svg
        ...
        <button type="button" onClick={addSkill} className="border border-secondary p-2 bg-white rounded">
