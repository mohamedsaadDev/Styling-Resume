...
const CustomSection2 = ({removeCustomSectionByIndex,index}) => {
  const {section2,setSection2} = useContext(Context);
  ...
  <Accordion.Item eventKey="6">
    <Accordion.Header className="btn-choose constainer-header-accordion">
      ...
      <button
        ...
        type="button"
      >
        <input className="border-0 " onChange={(e)=>{updateSection("title",e)}} value={section2.title} type="text"/>
    ...
    <Accordion.Body>
      {section2.elements.map((filed,index)=>
      <div>
      {filed === 'input'? 
      <> <input 
      value={section2.valueElements[index]}
      onChange={(e)=>{
