...
const CustomSection3 = ({removeCustomSectionByIndex,index}) => {
  const {section3,setSection3} = useContext(Context);
  const updateSection = (key,e) => {
    setSection3((prevSection) => ({
      ...prevSection,
      [key]: e.target.value,
    }));
  }
  ...
  return (
    ...
    <Accordion.Header className="btn-choose constainer-header-accordion">
      ...
      <button
        ...
        type="button"
      >
        <input className="border-0 " onChange={(e)=>{updateSection("title",e)}} value={section3.title} type="text"/>
    ...
    <Accordion.Body>
      {section3.elements.map((filed,index)=>
      <div>
      {filed === 'input'? 
      <> <input 
      value={section3.valueElements[index]}
      onChange={(e)=>{
