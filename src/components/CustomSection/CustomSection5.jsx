...
const CustomSection5 = ({removeCustomSectionByIndex,index}) => {
  const {section5,setSection5} = useContext(Context);
  const updateSection = (key,e) => {
    setSection5((prevSection) => ({
      ...prevSection,
      [key]: e.target.value,
    }));
  }
  ...
  return (
    ...
    <Accordion.Header className="btn-choose constainer-header-accordion">
      ...
      <button
        ...
        type="button"
      >
        <input className="border-0 " onChange={(e)=>{updateSection("title",e)}} value={section5.title} type="text"/>
        <div className="d-flex align-items-center">
          <Dropdown onClick={(e) => e.stopPropagation()}>
            ...
            <Dropdown.Menu>
              ...
              <Dropdown.Item onClick={()=>{removeCustomSectionByIndex(index)}}>
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 23 26"
    ...
    <Accordion.Body>
      {section5.elements.map((filed,index)=>
      <div>
      {filed === 'input'? 
      <> <input 
      value={section5.valueElements[index]}
      onChange={(e)=>{
    ...
    <Dropdown>
      <Dropdown.Toggle className="btn-dropdown-CustomSection"  id="dropdown-basic">
      <i className="fa-solid fa-plus"></i> Add Filed
      </Dropdown.Toggle>
      <Dropdown.Menu>
        <Dropdown.Item onClick={()=>{AddElement('input')}} >input</Dropdown.Item>
        <Dropdown.Item onClick={()=>{AddElement('list')}} >list</Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown>
...
// git remote add origin https://github.com/mohamedsaadDev/Styling-Resume.git