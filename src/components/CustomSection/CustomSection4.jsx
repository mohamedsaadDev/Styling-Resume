...
const CustomSection4 = ({removeCustomSectionByIndex,index}) => {
  const {section4,setSection4} = useContext(Context);
  const updateSection = (key,e) => {
    setSection4((prevSection) => ({
      ...prevSection,
      [key]: e.target.value,
    }));
  }
  ...
  return (
    ...
    <Accordion.Header className="btn-choose constainer-header-accordion">
      ...
      <button
        ...
        type="button"
      >
        <input className="border-0 " onChange={(e)=>{updateSection("title",e)}} value={section4.title} type="text"/>
    ...
    <Accordion.Body>
      {section4.elements.map((filed,index)=>
      <div>
      {filed === 'input'? 
      <> <input 
      value={section4.valueElements[index]}
      onChange={(e)=>{
